...
public interface WtmTaaApplDetRepository extends JpaRepository<WtmTaaApplDet, Long> {
	...
	@Query("SELECT D " +
			" FROM WtmTaaAppl A " +
			" JOIN WtmTaaApplDet D " +
			"   ON D.taaApplId = A.taaApplId " +
			" LEFT OUTER JOIN WtmTaaCanAppl C " +
			"   ON C.tenantId = A.tenantId " +
			"  AND C.enterCd = A.enterCd " +
			"  AND C.canApplId = A.applId " +
			" LEFT OUTER JOIN WtmAppl CA " +
			"   ON CA.applId = C.applId " +
			" LEFT OUTER JOIN WtmAppl WA " +
			"   ON WA.applId = A.applId " +
			"WHERE C.canApplId IS NULL " +
			"  AND A.tenantId = :tenantId " +
			"  AND A.enterCd = :enterCd " +
			"  AND A.sabun = :sabun " +
			"  AND  :ymd BETWEEN D.symd AND D.eymd " +
			"  AND WA.applStatusCd in ('99', '44') " +
			"  AND A.ifApplNo in (SELECT MAX(W.ifApplNo)  " +
			"                       FROM WtmTaaAppl W " +
			"                       JOIN WtmTaaApplDet T " +
			"                         ON W.taaApplId = T.taaApplId " +
			"                      WHERE W.tenantId = A.tenantId " +
			"                        AND W.enterCd = A.enterCd " +
			"                        AND W.sabun = A.sabun " +
			"                        AND T.taaCd = D.taaCd " +
			"                        AND  T.symd = D.symd " +
			"GROUP BY W.tenantId, W.enterCd, W.sabun, T.taaCd,  T.symd, W.ifApplNo ) ")
